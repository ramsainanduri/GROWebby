...
export function PlotCard({ jobName, metrics, series, stageFilter }: { jobName: string; metrics: SimulationJob["metrics"]; series: PlotSeries; stageFilter: string }) {
    ...
    <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-soft dark:border-slate-800 dark:bg-slate-900">
      <div className="mb-3 grid gap-3 2xl:grid-cols-[minmax(0,1fr)_auto]">
        <div className="min-w-0">
          <input className="mb-2 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm focus:border-ocean-500 focus:ring-1 focus:ring-ocean-500 dark:border-slate-700 dark:bg-slate-800" value={title} onChange={(event) => setTitle(event.target.value)} aria-label={`${series.label} plot title`} />
        ...
        <div className="grid gap-2 sm:grid-cols-2 2xl:min-w-[360px]">
          ...
          <select className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm focus:border-ocean-500 focus:ring-1 focus:ring-ocean-500 dark:border-slate-700 dark:bg-slate-800 h-9" value={scaleMode} onChange={(event) => setScaleMode(event.target.value)} aria-label={`${series.label} y scale`}>
          ...
          <div className="flex gap-2">
            <button type="button" onClick={() => setShowGrid((value) => !value)} className={`rounded-lg border border-slate-300 bg-white px-3 py-1 text-sm font-medium text-slate-700 shadow-sm transition-colors hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 h-9 flex-1 ${showGrid ? "border-ocean-400 text-ocean-700 dark:border-ocean-500 dark:text-ocean-400" : ""}`}>Grid</button>
            <button type="button" onClick={() => setShowPoints((value) => !value)} className={`rounded-lg border border-slate-300 bg-white px-3 py-1 text-sm font-medium text-slate-700 shadow-sm transition-colors hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 h-9 flex-1 ${showPoints ? "border-ocean-400 text-ocean-700 dark:border-ocean-500 dark:text-ocean-400" : ""}`}>Points</button>
          </div>
        </div>
      </div>
      ...
      <div className="mt-3 flex flex-wrap justify-end gap-2">
        <button type="button" className="rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-sm font-medium text-slate-700 shadow-sm transition-colors hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700" onClick={() => downloadCsv(`${jobName}-${series.key}-${stageFilter}.csv`, chartData, series.key)}>Export CSV</button>
        <button type="button" className="rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-sm font-medium text-slate-700 shadow-sm transition-colors hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700" onClick={() => downloadChartSvg(`${jobName}-${series.key}-${stageFilter}.svg`, chartRef.current)}>Export SVG</button>
      </div>
    </div>
  );
}
