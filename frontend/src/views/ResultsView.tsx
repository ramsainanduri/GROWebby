...
import { ArtifactFile, logHistoryUrl, readArtifact, saveArtifact, SimulationJob } from "../lib/api";
import { ViewKey, StepKey } from "../types";
import { downloadAllMetricsCsv, executionModeLabel, formatRunTime, statusClass, stepName } from "../lib/utils";
...
export function ResultsView({ cancelRun, configureNextStep, job, logs, notify, renameRun, selectRun, dark }: ResultsProps) {
      ...
      <section className="flex min-h-0 flex-col gap-4">
        <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-soft dark:border-slate-800 dark:bg-slate-900">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div className="min-w-0 flex-1">
              <div className="flex max-w-xl items-center gap-2">
                <input className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm focus:border-ocean-500 focus:ring-1 focus:ring-ocean-500 dark:border-slate-700 dark:bg-slate-800" value={runNameDraft} onChange={(event) => setRunNameDraft(event.target.value)} />
                <button type="button" className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm transition-colors hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 h-[2.35rem] whitespace-nowrap" onClick={() => renameRun(job.id, runNameDraft)}>Rename</button>
              </div>
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{job.upload?.originalName}</p>
            </div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <button type="button" className="rounded-lg bg-ocean-600 px-4 py-2 text-sm font-semibold text-white shadow-soft transition-colors hover:bg-ocean-700 disabled:opacity-50" onClick={() => configureNextStep(job)} disabled={activeStepKey === "production"}>Configure next step</button>
            <button type="button" className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm transition-colors hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700" onClick={() => selectRun(job.id, "results")}>Refresh run</button>
          </div>
          <div className="mt-4 flex max-w-2xl flex-wrap items-end gap-3">
            ...
            <button type="button" className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm transition-colors hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 h-[2.35rem]" onClick={() => downloadAllMetricsCsv(`${job.name}-metrics-${stageFilter}.csv`, plottedMetrics)}>Export All Metrics</button>
          </div>
        </div>
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-soft dark:border-slate-800 dark:bg-slate-900">
            <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex gap-2">
                {selectedArtifact && <a className="rounded-lg border border-slate-300 bg-white px-3 py-1 text-sm font-medium text-slate-700 shadow-sm transition-colors hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700" href={selectedArtifact.url} target="_blank" rel="noreferrer">Open</a>}
                <button type="button" className="rounded-lg bg-ocean-600 px-3 py-1 text-sm font-semibold text-white shadow-soft transition-colors hover:bg-ocean-700 disabled:opacity-50" disabled={!selectedArtifact || artifactBusy} onClick={async () => {
